...
// ISO 8601 → MySQL DATETIME 변환 ('2026-03-18T12:08:44.184Z' → '2026-03-18 12:08:44')
...
// Date 객체 → 'YYYY-MM-DD HH:mm:ss' 문자열 (프론트엔드 호환)
...
// 데드락 재시도 헬퍼 (DELETE→INSERT 패턴용)
async function withDeadlockRetry(fn, label) {
  const MAX_RETRIES = 3;
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      return await fn();
    } catch(e) {
      if (e.code === 'ER_LOCK_DEADLOCK' && attempt < MAX_RETRIES - 1) {
        console.error('[DAL] Deadlock (' + (label || '') + '), retry ' + (attempt + 1));
...
function flattenForSave(nodes, parentId, result, sortStart) {
  ...
  for (const n of nodes) {
    result.push({
      ...
      last_login_at: toMySQLDate(n.lastLoginAt), last_login_ip: n.lastLoginIp || null,
      registered_at: toMySQLDate(n.registeredAt), expanded: n.expanded ? 1 : 0, sort_order: sort++
    });
...
const users = {
  ...
  async create(user) {
    return db.run(
      ...
      [user.id, user.username, user.nickname, user.password,
       ...
       toMySQLDate(user.registeredAt), toMySQLDate(user.approvedAt),
       toMySQLDate(user.lastLoginAt), user.lastLoginIp || null]
    );
  },
  async update(id, fields) {
    const keys = Object.keys(fields);
  ...
  // 원자적 머니 증감 (race condition 방지)
  async addMoney(username, amount) {
    return db.run('UPDATE users SET money = GREATEST(0, money + ?) WHERE username = ?', [amount, username]);
  },
  async setMoney(username, money) {
    return db.run('UPDATE users SET money = ? WHERE username = ?', [money, username]);
  },
  async getMoney(username) {
    const row = await db.getOne('SELECT money FROM users WHERE username = ?', [username]);
    return row ? (row.money || 0) : 0;
  },
  async addPoint(username, amount) {
    return db.run('UPDATE users SET point = GREATEST(0, point + ?) WHERE username = ?', [amount, username]);
  },
  async getPoint(username) {
    const row = await db.getOne('SELECT point, rolling_point FROM users WHERE username = ?', [username]);
    return row ? { point: row.point || 0, rollingPoint: row.rolling_point || 0 } : { point: 0, rollingPoint: 0 };
  },
  async delete(id) {
        ...
        for (const u of arr) {
          await conn.query(
            ...
            [u.id, u.username, u.nickname, u.password,
             ...
             toMySQLDate(u.registeredAt), toMySQLDate(u.approvedAt),
             toMySQLDate(u.lastLoginAt), u.lastLoginIp || null]
          );
        }
        await conn.commit();
      } catch(e) {
  ...
  return {
    ...
    async create(tx) {
      return db.run(
        ...
        [tx.id, tx.type, tx.amount || 0, tx.before || 0, tx.status,
         JSON.stringify(tx.details || {}), toMySQLDate(tx.processed_at),
         tx.referer_id || null, toMySQLDate(tx.created_at),
         typeof tx.user === 'object' ? JSON.stringify(tx.user) : (tx.user || null),
    ...
    async readAll() {
      ...
      return rows.map(r => ({
        ...
        user: typeof r.user === 'string' ? JSON.parse(r.user) : r.user,
          ...
          for (const t of arr) {
            await conn.query(
              ...
              [t.id, t.type, t.amount || 0, t.before || 0, t.status,
               JSON.stringify(t.details || {}), toMySQLDate(t.processed_at),
               t.referer_id || null, toMySQLDate(t.created_at),
               typeof t.user === 'object' ? JSON.stringify(t.user) : (t.user || null),
...
const transfers = {
  ...
  async readAll() {
    ...
    return rows.map(r => ({
      ...
      userId: r.username,
      nick: r.nickname,
      datetime: toDateStr(r.created_at),
      processedAt: toDateStr(r.processed_at),
      ...
      try {
        ...
        for (const t of arr) {
          await conn.query(
            `INSERT INTO transfers (type, username, nickname, amount, bank, account, holder, status, memo, processed_by, created_at, processed_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
            [t.type, t.username || t.userId || '', t.nickname || t.nick || '', t.amount || 0, t.bank || '', t.account || '',
             t.holder || '', t.status || 'pending', t.memo || '', t.processedBy || t.processed_by || null,
             toMySQLDate(t.createdAt || t.created_at || t.datetime), toMySQLDate(t.processedAt || t.processed_at)]
          );
        }
        await conn.commit();
...
function moneyLogHelper(logType) {
  return {
    ...
    async readAll() {
      ...
      return rows.map(r => ({
        ...r,
        processorLevel: r.processor_level,
...
const moneyLogRollingConvert = moneyLogHelper('rolling-convert');
...
const loginLogs = {
  ...
  async readAll() { return this.getAll(); }, // login_logs columns are already camelCase-compatible
...
const notices = {
  ...
  async readAll() {
    ...
    return rows.map(r => ({
      ...r,
      partnerPopup: !!r.partner_popup,
      userPopup: !!r.user_popup,
      loginPopup: !!r.login_popup,
      partnerShow: !!r.partner_show,
      userShow: !!r.user_show,
        ...
        for (const n of arr) {
          await conn.query(
            ...
            [n.title, n.content, n.image || '', (n.partnerPopup || n.partner_popup) ? 1 : 0,
             ...
             n.rank || 0, n.domain || '', toMySQLDate(n.createdAt || n.created_at)]
          );
        }
        await conn.commit();
      } catch(e) { await conn.rollback(); throw e; }
...
const events = {
  ...
  async create(e) {
    return db.run(`INSERT INTO events (title, content, image, status, created_at) VALUES (?,?,?,?,?)`,
      [e.title, e.content, e.image || '', e.status || 'active', toMySQLDate(e.createdAt || new Date())]);
  },
  async update(id, fields) {
    const keys = Object.keys(fields);
    const sets = keys.map(k => `\`${k}\` = ?`).join(', ');
  ...
  async readAll() {
    const rows = await this.getAll();
    return rows.map(r => ({ ...r, createdAt: toDateStr(r.created_at) }));
  },
  async writeAll(arr) {
    await withDeadlockRetry(async () => {
      const conn = await db.getConnection();
      try {
        ...
        for (const e of arr) {
          await conn.query(`INSERT INTO events (title, content, image, status, created_at) VALUES (?,?,?,?,?)`,
            [e.title, e.content, e.image || '', e.status || 'active', toMySQLDate(e.createdAt || e.created_at)]);
        }
        await conn.commit();
...
const inquiries = {
  ...
  async create(q) {
    return db.run(`INSERT INTO inquiries (username, title, content, reply, status, created_at, replied_at) VALUES (?,?,?,?,?,?,?)`,
      [q.username, q.title, q.content, q.reply || null, q.status || 'pending', toMySQLDate(q.createdAt || new Date()), toMySQLDate(q.repliedAt)]);
  },
  async update(id, fields) {
    const keys = Object.keys(fields);
    const sets = keys.map(k => `\`${k}\` = ?`).join(', ');
  ...
  async readAll() {
    ...
    return rows.map(r => ({
      ...
      datetime: toDateStr(r.created_at),
      answer: r.reply,
      answeredAt: toDateStr(r.replied_at)
    }));
  },
  async writeAll(arr) {
    await withDeadlockRetry(async () => {
      ...
      try {
        ...
        for (const q of arr) {
          await conn.query(`INSERT INTO inquiries (username, title, content, reply, status, created_at, replied_at) VALUES (?,?,?,?,?,?,?)`,
            [q.username, q.title, q.content, q.reply || null, q.status || 'pending', toMySQLDate(q.createdAt || q.created_at), toMySQLDate(q.repliedAt || q.replied_at)]);
        }
        await conn.commit();
      } catch(e) { await conn.rollback(); throw e; }
      finally { conn.release(); }
...
const messages = {
  ...
  async create(m) {
    return db.run(`INSERT INTO messages (sender, receiver, title, content, is_read, created_at) VALUES (?,?,?,?,?,?)`,
      [m.sender, m.receiver, m.title, m.content, 0, toMySQLDate(m.createdAt || new Date())]);
  },
      ...
      try {
        ...
        for (const m of arr) {
          await conn.query(`INSERT INTO messages (sender, receiver, title, content, is_read, created_at) VALUES (?,?,?,?,?,?)`,
            [m.sender, m.receiver, m.title, m.content, m.isRead || m.is_read ? 1 : 0, toMySQLDate(m.createdAt || m.created_at)]);
        }
        await conn.commit();
      } catch(e) { await conn.rollback(); throw e; }
      finally { conn.release(); }
...
const referrals = {
  ...
  async create(r) {
    return db.run(`INSERT INTO referrals (id, user_id, code, used_count, used_by, created_at) VALUES (?,?,?,?,?,?)`,
      [r.id, r.userId, r.code, r.usedCount || 0, JSON.stringify(r.usedBy || []), toMySQLDate(r.createdAt || new Date())]);
  },
  async update(id, fields) {
    const keys = Object.keys(fields);
    const sets = keys.map(k => `\`${k}\` = ?`).join(', ');
      ...
      try {
        ...
        for (const r of arr) {
          await conn.query(`INSERT INTO referrals (id, user_id, code, used_count, used_by, created_at) VALUES (?,?,?,?,?,?)`,
            [r.id, r.userId || r.user_id, r.code, r.usedCount || r.used_count || 0, JSON.stringify(r.usedBy || r.used_by || []), toMySQLDate(r.createdAt || r.created_at)]);
        }
        await conn.commit();
      } catch(e) { await conn.rollback(); throw e; }
      finally { conn.release(); }
...
const quickReplies = {
  ...
  async create(q) {
    return db.run(`INSERT INTO quick_replies (title, content, created_at) VALUES (?,?,?)`,
      [q.title, q.content, toMySQLDate(q.createdAt || new Date())]);
  },
  async update(id, fields) {
    const keys = Object.keys(fields);
    const sets = keys.map(k => `\`${k}\` = ?`).join(', ');
      ...
      try {
        ...
        for (const q of arr) {
          await conn.query(`INSERT INTO quick_replies (title, content, created_at) VALUES (?,?,?)`,
            [q.title, q.content, toMySQLDate(q.createdAt || q.created_at)]);
        }
        await conn.commit();
      } catch(e) { await conn.rollback(); throw e; }
      finally { conn.release(); }
...
const adminSettings = {
  async get(key) {
    ...
    return row ? (typeof row.setting_value === 'string' ? JSON.parse(row.setting_value) : row.setting_value) : null;
  },
  async set(key, value) {
    return db.run(
  ...
  async readAll() {
    ...
    const obj = {};
    rows.forEach(r => { obj[r.setting_key] = typeof r.setting_value === 'string' ? JSON.parse(r.setting_value) : r.setting_value; });
    return obj;
  },
...
const gamesConfig = {
  async get(key) {
    ...
    return row ? (typeof row.config_value === 'string' ? JSON.parse(row.config_value) : row.config_value) : null;
  },
  async set(key, value) {
    return db.run(
  ...
  async readAll() {
    ...
    const obj = {};
    rows.forEach(r => { obj[r.config_key] = typeof r.config_value === 'string' ? JSON.parse(r.config_value) : r.config_value; });
    return obj;
  },
  async writeAll(obj) {
    await withDeadlockRetry(async () => {
...
const fileMap = {
  ...
  'money_log_rolling-convert.json': moneyLogRollingConvert,
...
module.exports = {
  ...
  transfers, moneyLogAdmin, moneyLogPartner, moneyLogUser, moneyLogPoint, moneyLogRollingConvert,
