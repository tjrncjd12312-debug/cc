...
const DATA_FILE    = path.join(__dirname, '..', 'data', 'cs_transactions.json');
...
async function collect() {
  ...
  try {
    ...
    if (newBets.length > 0) {
      try { processRolling(newBets); } catch(e) { console.error('[CSTransactionCollector] Rolling error:', e.message); }
    }
