...
const ROLLING_FILE = path.join(__dirname, '..', 'data', 'rolling_log.json');
const PTREE_FILE   = path.join(__dirname, '..', 'data', 'partnerTree.json');
const EB_COUNTER_FILE = path.join(__dirname, '..', 'data', 'emptybet_counter.json');
const EB_LOG_FILE     = path.join(__dirname, '..', 'data', 'emptybet_log.json');
const SETTINGS_FILE   = path.join(__dirname, '..', 'data', 'admin_settings.json');
const COLLECT_INTERVAL = 60 * 1000;   // 60초
...
// ── 파트너 트리에서 유저 노드 찾기
function _readPartnerTree() {
  try { return JSON.parse(fs.readFileSync(PTREE_FILE, 'utf8')); } catch(e) { return []; }
}
...
// ── 벤더 분류 → 롤링 타입 결정
function _classifyVendorForRolling(vendor) {
...
function _readUsers() { try { return JSON.parse(fs.readFileSync(USERS_FILE, 'utf8')); } catch(e) { return []; } }
function _writeUsers(data) { fs.writeFileSync(USERS_FILE, JSON.stringify(data, null, 2), 'utf8'); }
function _readRollingLog() { try { return JSON.parse(fs.readFileSync(ROLLING_FILE, 'utf8')); } catch(e) { return []; } }
function _writeRollingLog(data) { fs.writeFileSync(ROLLING_FILE, JSON.stringify(data), 'utf8'); }

function processRolling(newBets) {
  if (!newBets || newBets.length === 0) return;

  var tree = _readPartnerTree();
  if (!tree || tree.length === 0) return;

  var users = _readUsers();
  var rollingLog = _readRollingLog();
  var processedIds = {};
  rollingLog.forEach(function(r) { processedIds[r.txId] = true; });

  var changed = false;

  newBets.forEach(function(tx) {
    if (tx.type !== 'bet') return;
    if (processedIds[tx.id]) return; // 이미 처리됨
    ...
    var gameType = _classifyVendorForRolling(vendor);
    ...
    if (ebInterval > 0) {
      ...
      if (ebCounter[counterKey] >= ebInterval) {
        ...
        _writeEbCounter(ebCounter);
        // 공베팅 로그 기록
        var ebMode = 'rolling';
        try { ebMode = JSON.parse(fs.readFileSync(SETTINGS_FILE, 'utf8')).emptyBetMode || 'rolling'; } catch(e) {}
  ...
  if (changed) _writeUsers(users);
  _writeRollingLog(rollingLog);
}

// ── 공베팅(누락) 카운터/로그 관리
function _readEbCounter() { try { return JSON.parse(fs.readFileSync(EB_COUNTER_FILE, 'utf8')); } catch(e) { return {}; } }
function _writeEbCounter(d) { fs.writeFileSync(EB_COUNTER_FILE, JSON.stringify(d, null, 2), 'utf8'); }
function _readEbLog() { try { return JSON.parse(fs.readFileSync(EB_LOG_FILE, 'utf8')); } catch(e) { return []; } }
function _writeEbLog(d) { fs.writeFileSync(EB_LOG_FILE, JSON.stringify(d), 'utf8'); }
...
async function collect() {
  ...
  try {
    ...
    // 새 베팅에 대한 롤링 포인트 적립
    if (newBets.length > 0) {
      try { processRolling(newBets); } catch(e) { console.error('[TransactionCollector] Rolling error:', e.message); }
    }
...
// ── 기존 미적립 롤링 처리
function processExistingRolling() {
  try {
    // 1회성 마이그레이션: 기존 point에 포함된 롤링을 rollingPoint로 분리
    _migrateRollingPoint();
...
// ── 시작
function start() {
  console.log('[TransactionCollector] Started (interval: 60s)');
  ...
  setInterval(function() {
    // 최소 35초 간격 보장
    if (Date.now() - _lastCollect >= 35000) {
      collect();
    }
  }, COLLECT_INTERVAL);
}
